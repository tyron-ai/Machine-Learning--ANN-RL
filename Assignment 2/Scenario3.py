...
def Qlearning(vis: np.array([[int]]), ep: int):
    ...
    packageSequence = ['R', 'G', 'B']  # Desired sequence of package types
    ...
    while not end:
        ...
        # Check if the collected package matches the next package in the sequence
        if gType == packageSequence[packageCount]:
            packageCount += 1
